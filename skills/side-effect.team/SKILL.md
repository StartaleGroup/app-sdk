...
|---|---|
| **Zustand stores** | Store actions/selectors used by other components |
| **TanStack Query caches** | Query keys, cache shape, `select` transforms |
| **WSS subscriptions** | Channel names, payload schemas, merge logic |
...
For shared UI components (`components/ui/*`), check ALL consumers:
...
For changed utility functions (`lib/**`):
...
#### 2e. Layout & Spacing Cascade

For CSS/Tailwind changes:
- **Grid/flex changes**: Do `grid-cols-*` or `1fr` changes affect responsive breakpoints?
- **Fixed-width → flexible**: Does removing fixed widths cause content overflow or collapse?
