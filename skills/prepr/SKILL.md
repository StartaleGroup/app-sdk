...
4. **Check unit test coverage for new functions**: For each new exported
   function detected in `src/lib/` or `src/hooks/`:
   - Look for `__tests__/<filename>.test.ts(x)` in the same directory
   - If no test file exists, add function to "missing tests" list
   - Skip functions in `api/` dirs (generated), `types/` dirs, route
     files, and UI components
...
- For each new function/helper in the PR, check `src/lib/` for existing
...
- **Scope**: Only files that change exports, shared state (Zustand stores,
  TanStack Query keys, WSS subscriptions, Context providers,
  localStorage/sessionStorage), or shared UI components (`components/ui/*`)
...
5. **Apply remaining fixes**: For issues found by Agent 2, 3, and 4 that
...
7. **Run simplification** (invoke `/simplify` skill or `code-simplifier` agent):
...
10. **Run `pnpm biome:fix` and `pnpm type-check` in parallel** (two
    separate Bash calls in one message)
11. **Fix errors**: If either fails, fix reported errors and re-run
12. **Run `pnpm build`**: Execute build after biome + type-check pass
...
- Agent 2, 3, and 4 are read-only reviewers; only Agent 1 (code-reviewer)
...
- Step 4 runs biome:fix and type-check in parallel to reduce validation time
- Agent 3 and Agent 4 are conditionally launched based on Step 1 classification:
