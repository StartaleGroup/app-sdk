			...
			switch (args.method) {
				...
				case 'wallet_sendCalls': {
						...
						params: [
							{
								...
								capabilities:
									'capabilities' in args.params[0]
										? (args.params[0].capabilities as Record<
												string,
												unknown
											>)
										: {},
					...
					const signResponse = await owner!.sign?.({
						...
						hash: hexToString(
							prepareCallsResponse.signatureRequest.hash,
						) as `0x${string}`,
					})
				...
				case 'wallet_sendPreparedCalls': {
					...
					const sendPreparedCallsResponse =
						await client.request<SendPreparedCallsSchema>({
							method: 'wallet_sendPreparedCalls',
							params:
								args.params as SendPreparedCallsSchema['Parameters'],
						})
				...
				case 'wallet_prepareCalls': {
					...
					const prepareCallsParams = args
						.params[0] as PrepareCallsSchema['Parameters'][0]
					...
					const prepareCallsResponse =
						await client.request<PrepareCallsSchema>({
							method: 'wallet_prepareCalls',
							params: [
								{ ...args.params[0], chainId: chainId },
							] as PrepareCallsSchema['Parameters'],
						})
