...
describe('handleAddSubAccountOwner', () => {
	...
	beforeEach(() => {
		...
		;(waitForCallsStatus as ReturnType<typeof vi.fn>).mockResolvedValue({
			status: 'success',
		})
		;(findOwnerIndex as ReturnType<typeof vi.fn>).mockResolvedValue(1)
		;(presentAddOwnerDialog as ReturnType<typeof vi.fn>).mockResolvedValue(
			'authenticate',
		)
	})
	...
	it('should throw error when client is not found', async () => {
		...
		await expect(
			...
			standardErrors.rpc.internal('client not found for chainId 1'),
		)
	})
	...
	it('should throw error when findOwnerIndex returns -1', async () => {
		...
		await expect(
			...
			standardErrors.rpc.internal('failed to find owner index'),
		)
	})
