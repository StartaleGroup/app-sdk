...
import {
	RPCRequestMessage,
	RPCResponseMessage,
} from ':core/message/RPCMessage.js'
...
export class Signer {
	...
	async _request(request: RequestArguments) {
		...
		switch (request.method) {
			...
			case 'wallet_connect': {
				...
				const hasSiweCapability = requestHasCapability(
					request,
					'signInWithEthereum',
				)
			...
			case 'coinbase_fetchPermissions': {
				...
				const requestedChainId = hexToNumber(
					completeRequest.params?.[0].chainId,
				)
	...
	private async handleGetCapabilitiesRequest(request: RequestArguments) {
		...
		const filteredCapabilities = Object.fromEntries(
			Object.entries(capabilities).filter(([capabilityKey]) => {
				try {
					const capabilityChainNumber = hexToNumber(
						capabilityKey as `0x${string}`,
					)
	...
	private async sendRequestToSubAccountSigner(request: RequestArguments) {
		...
		const globalAccountAddress = this.accounts.find(
			(account) =>
				account.toLowerCase() !== subAccount.address.toLowerCase(),
		)
		...
		if (
			['eth_sendTransaction', 'wallet_sendCalls'].includes(request.method)
		) {
