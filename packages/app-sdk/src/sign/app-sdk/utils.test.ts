...
describe('utils', () => {
	...
	describe('addSenderToRequest', () => {
		it.each([
			...
			['wallet_deterministicSign', ['hello', undefined], ['hello', '0x123']],
		])(
...
describe('assertGetCapabilitiesParams', () => {
	...
	it('should throw if params array has more than 2 elements', () => {
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_1, ['0x1'], 'extra']),
		).toThrow()
	})
	...
	it('should throw if first param is not a valid Ethereum address', () => {
		...
		expect(() =>
			assertGetCapabilitiesParams([
				'0x12345678901234567890123456789012345678gg',
			]),
	...
	it('should throw if second param is not an array when present', () => {
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_1, '0x1']),
		).toThrow()
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_1, 123]),
		).toThrow()
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_1, null]),
		).toThrow()
	...
	it('should throw if second param array contains non-hex strings', () => {
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_1, ['0x1', '123']]),
		).toThrow()
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_1, ['0x1', 123]]),
		).toThrow()
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_1, ['0x1', null]]),
		).toThrow()
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_1, ['abc123']]),
		).toThrow()
	})

	it('should not throw for valid parameters with filter array', () => {
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_1, []]),
		).not.toThrow()
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_1, ['0x1']]),
		).not.toThrow()
		...
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_1, ['0xabcdef', '0x0']]),
		).not.toThrow()
		expect(() =>
			assertGetCapabilitiesParams([VALID_ADDRESS_2, ['0x1', '0xa']]),
		).not.toThrow()
	})
})
...
describe('assertFetchPermissionsRequest', () => {
	it('should throw if the request is not a fetch permissions request', () => {
		expect(() =>
			assertFetchPermissionsRequest({ method: 'eth_sendTransaction' }),
		).toThrow()
	})
...
describe('requestCapabilities', () => {
	describe('requestHasCapability', () => {
		it('returns false for requests without params', () => {
			...
			expect(requestHasCapability({ params: null } as any, 'test')).toBe(
				false,
			)
		})
		...
		it('returns false for requests without capabilities', () => {
			...
			expect(requestHasCapability({ params: [{}] } as any, 'test')).toBe(
				false,
			)
		})
		...
		it('returns false when capabilities is not an object', () => {
			expect(
				requestHasCapability(
					{ params: [{ capabilities: 'not-an-object' }] } as any,
					'test',
				),
			).toBe(false)
		})
