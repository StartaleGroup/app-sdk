...
describe('createSpendPermissionTypedData', () => {
	...
	it('should have correct EIP-712 types structure', () => {
		...
		expect(result.types.SpendPermission).toContainEqual({
			name: 'account',
			type: 'address',
		})
		expect(result.types.SpendPermission).toContainEqual({
	...
	it('should have correct SpendPermission field order for EIP-712 compatibility', () => {
		...
		expect(result.types.SpendPermission[0]).toEqual({
			name: 'account',
			type: 'address',
		})
		expect(result.types.SpendPermission[1]).toEqual({
			name: 'spender',
			type: 'address',
		})
		expect(result.types.SpendPermission[2]).toEqual({
			name: 'token',
			type: 'address',
		})
		expect(result.types.SpendPermission[3]).toEqual({
			name: 'allowance',
			type: 'uint160',
		})
		expect(result.types.SpendPermission[4]).toEqual({
			name: 'period',
			type: 'uint48',
		})
		expect(result.types.SpendPermission[5]).toEqual({
			name: 'start',
			type: 'uint48',
		})
		expect(result.types.SpendPermission[6]).toEqual({
			name: 'end',
			type: 'uint48',
		})
		expect(result.types.SpendPermission[7]).toEqual({
			name: 'salt',
			type: 'uint256',
		})
		expect(result.types.SpendPermission[8]).toEqual({
			name: 'extraData',
			type: 'bytes',
		})
	})
...
describe('toSpendPermissionArgs', () => {
	...
	it('should handle different address formats and checksum them', () => {
		...
		expect(result.account).not.toBe(
			permission.permission.account.toLowerCase(),
		)
		expect(result.spender).not.toBe(
	...
	it('should handle different timestamps correctly', () => {
		const testCases = [
			...
			},
			{
				...
				expected: new Date('2021-12-31T23:59:59.000Z'),
			},
			{ timestamp: 0, expected: new Date('1970-01-01T00:00:00.000Z') },
			{
				timestamp: 2147483647,
				expected: new Date('2038-01-19T03:14:07.000Z'),
