...
export default function UserOps() {
	...
	const [getSubAccountsLoading, setGetSubAccountsLoading] =
		React.useState(false)
	const [addedSubAccount, setAddedSubAccount] = React.useState<string | null>(
		null,
	)
		...
		try {
			...
			const response = (await provider.request({
				...
				| {
						id?: string
						result?: {
							value?: { transactionHash?: string; userOpHash?: string }
						}
				  }
				| string
				| null
			...
			if (typeof response === 'string') {
				...
				if ('id' in response && typeof response.id === 'string') {
					...
					if (value && typeof value === 'object') {
						const txHash = (value as Record<string, unknown>)
							.transactionHash
						if (typeof txHash === 'string') {
							extractedHash = txHash
						}
						const userOpHashValue = (value as Record<string, unknown>)
							.userOpHash
	...
	const handleGetSubAccounts = async () => {
		...
		try {
			...
			if (response && typeof response === 'object') {
				if (
					'subAccounts' in response &&
					Array.isArray(response.subAccounts)
				) {
					...
					if (
						value &&
						typeof value === 'object' &&
						'subAccounts' in value
					) {
						const subAccounts = (value as Record<string, unknown>)
							.subAccounts
