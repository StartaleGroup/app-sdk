		...
		async (response: ResponseType, data: Record<string, string>) => {
			...
			const chain =
				multiChainShortcutsMap['wallet_switchEthereumChain'].find(
					(shortcut) =>
						Number(shortcut.data.chainId) === hexToNumber(chainId),
				)?.data.chain ?? soneium
						...
						{(() => {
							const extractTransactionHash = (
								...
								if (typeof value === 'object') {
									...
									const nested = record.result as
										| Record<string, unknown>
										| undefined
									if (nested && typeof nested === 'object') {
										const nestedValue = nested.value as
											| Record<string, unknown>
											| undefined
										if (
											nestedValue &&
											typeof nestedValue.transactionHash === 'string'
										) {
				...
				{verifyResult && (
					<VStack mt={4}>
						<Code
							...
							p={4}
							colorScheme={
								verifyResult.includes('Failed') ? 'red' : 'cyan'
							}
							wordBreak="break-word"
							whiteSpace="pre-wrap"
							w="100%"
