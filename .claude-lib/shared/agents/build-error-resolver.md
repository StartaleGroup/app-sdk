...
# Vite build (runs type-check first)
pnpm build
...
4. **Verify** — `pnpm validate` passes with no new errors
...
// ✅ OR: Use assert from ~/lib/utils for required values
...
// ✅ FIX 1: Verify path alias in tsconfig (~/*)
// ✅ FIX 2: Check actual export exists in target file
// ✅ FIX 3: Install missing package
pnpm add <package-name>
```

> **Always use path alias `~/*`** — never use relative imports like `../lib/utils`.
...
After all fixes:
1. `pnpm type-check` — exits with code 0
2. `pnpm build` — completes successfully
3. `pnpm biome` — no linting errors
